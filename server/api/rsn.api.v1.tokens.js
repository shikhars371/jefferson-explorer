...
module.exports 	= function(router, log, MARIA) {
	...
	router.post('/api/v1/get_account_tokens', (req, res) => {
		 let account = req.body.account;
	   	 MARIA.query(`select * from (
        			      select * from ARISEN_CURRENCY_BALANCES where account_name = :account and currency<>'RSN'
	...
	router.get('/api/v1/get_tokens', (req, res) => {
		 ...
		 CACHE_TIME = dateNow + 3600000; // cahce for hour
	   	 MARIA.query(`select * from (select currency, issuer, count(*) as rate from ARISEN_CURRENCY_BALANCES
			   			where currency<>'RSN'
			   			group by currency
